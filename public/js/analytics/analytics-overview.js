...
function renderOverviewTab(data) {
  ...
  const insightSummaryHtml = `
    ...
    <div class="bg-slate-900 dark:bg-black rounded-3xl p-6 text-white shadow-md flex flex-col lg:flex-row justify-between items-center gap-6 border border-slate-800 dark:border-slate-900 relative overflow-hidden select-none">
      ...
      <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-${showPeakTimeframe ? '7' : '6'} gap-4 w-full lg:w-auto shrink-0 z-10">
        ...
        <div class="bg-slate-950/40 dark:bg-slate-900/40 rounded-2xl p-4 border border-slate-800 dark:border-slate-800/60 flex flex-col justify-between min-w-[130px]">
          <span class="text-[10px] uppercase font-black tracking-wider text-slate-400 dark:text-slate-500">Total Period Sales</span>
        ...
        <div class="bg-slate-950/40 dark:bg-slate-900/40 rounded-2xl p-4 border border-slate-800 dark:border-slate-800/60 flex flex-col justify-between min-w-[130px]">
          <span class="text-[10px] uppercase font-black tracking-wider text-slate-400 dark:text-slate-500">Total Orders</span>
        ...
        <div class="bg-slate-950/40 dark:bg-slate-900/40 rounded-2xl p-4 border border-slate-800 dark:border-slate-800/60 flex flex-col justify-between min-w-[130px]">
          <span class="text-[10px] uppercase font-black tracking-wider text-slate-400 dark:text-slate-500">Total Customers</span>
        ...
        <div class="bg-slate-950/40 dark:bg-slate-900/40 rounded-2xl p-4 border border-slate-800 dark:border-slate-800/60 flex flex-col justify-between min-w-[130px]">
          <span class="text-[10px] uppercase font-black tracking-wider text-slate-400 dark:text-slate-500">Total Invoices</span>
          <h5 class="text-base font-black mt-1.5">${formatNum(k.totalInvoices)}</h5>
          ${formatGrowth(g.invoices)}
        </div>
 
        <!-- Metric 5: Peak Revenue Day/Month -->
        ${showPeakTimeframe ? `
          <div class="bg-slate-950/40 dark:bg-slate-900/40 rounded-2xl p-4 border border-slate-800 dark:border-slate-800/60 flex flex-col justify-between min-w-[130px]">
            <span class="text-[10px] uppercase font-black tracking-wider text-slate-400 dark:text-slate-500">${analyticsPeriod === '12months' ? 'Highest Month' : 'Highest Day'}</span>
        ...
        <!-- Metric 6: Best Selling Item -->
        <div class="bg-slate-950/40 dark:bg-slate-900/40 rounded-2xl p-4 border border-slate-800 dark:border-slate-800/60 flex flex-col justify-between min-w-[130px]">
          <span class="text-[10px] uppercase font-black tracking-wider text-slate-400 dark:text-slate-500">Best-Selling Item</span>
          <h5 class="text-xs font-black mt-1.5 truncate" title="${bestSellingProduct}">${bestSellingProduct}</h5>
        </div>
 
        <!-- Metric 7: Product Contribution Revenue -->
        <div class="bg-slate-950/40 dark:bg-slate-900/40 rounded-2xl p-4 border border-slate-800 dark:border-slate-800/60 flex flex-col justify-between min-w-[130px]">
          <span class="text-[10px] uppercase font-black tracking-wider text-slate-400 dark:text-slate-500">Item Revenue</span>
  ...
  viewport.innerHTML = `
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6 animate-[fadeIn_0.3s_ease-out]">
      ...
      <div class="w-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800/80 p-6 rounded-3xl shadow-sm flex flex-col justify-between h-fit">
        <div class="flex items-center justify-between mb-4">
          <div>
            <h5 class="text-xs font-black uppercase text-slate-400 tracking-wider">Sales Overview</h5>
      ...
      <div class="w-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800/80 p-6 rounded-3xl shadow-sm flex flex-col justify-between h-fit">
        <div class="flex items-center justify-between mb-4">
          <div>
            <h5 class="text-xs font-black uppercase text-slate-400 tracking-wider">Sales Trend</h5>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6 animate-[fadeIn_0.3s_ease-out]">
      ...
      <div class="w-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800/80 p-6 rounded-3xl shadow-sm h-fit">
        <div class="mb-4 flex items-center justify-between">
          <div>
            <h5 class="text-xs font-black uppercase text-slate-400 tracking-wider">Activity Heatmap</h5>
      ...
      <div class="w-full flex flex-col gap-6">
        ...
        <div class="w-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800/80 p-5 rounded-3xl shadow-sm flex flex-col justify-between h-fit">
          <div>
            <h5 class="text-xs font-black uppercase text-slate-400 tracking-wider">Sales by Category</h5>
        ...
        <div class="w-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800/80 p-5 rounded-3xl shadow-sm flex flex-col justify-between h-fit">
          <div>
            <h5 class="text-xs font-black uppercase text-slate-400 tracking-wider">Sales by Channel</h5>
        ...
        <div class="w-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800/80 p-5 rounded-3xl shadow-sm flex flex-col justify-between h-fit">
          <div>
            <h5 class="text-xs font-black uppercase text-slate-400 tracking-wider">Sales by Payment Method</h5>
    ...
    <div class="mb-6 animate-[fadeIn_0.3s_ease-out]">
      <div class="w-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800/80 p-6 rounded-3xl shadow-sm flex flex-col justify-between h-fit">
        <div>
          <h5 class="text-xs font-black uppercase text-slate-400 tracking-wider">Top Selling Products</h5>
...
function renderOverviewHeatmap(containerId, data) {
  ...
  const datesList = [];
  const startD = new Date(data.bounds.start.split(' ')[0]);
  const endD = new Date(data.bounds.end.split(' ')[0]);
  ...
  for (let i = 0; i < numDays; i++) {
    ...
    const m = String(d.getMonth() + 1).padStart(2, '0');
    const r = String(d.getDate()).padStart(2, '0');
    datesList.unshift(`${y}-${m}-${r}`); // Show newest days first
  }
  ...
  if (data.heatmapRaw) {
    data.heatmapRaw.forEach(r => {
      if (heatmapData[r.dt]) {
        const blockIdx = Math.min(Math.max(r.block_idx, 0), 5);
  ...
  const headerHtml = `
    <div class="flex items-center gap-2 md:gap-4 mb-2">
      <div class="w-20 md:w-28 shrink-0"></div>
      <div class="flex-1 flex items-center gap-1.5 md:gap-3">
        ${shortLabels.map(lbl => `<div class="flex-1 text-[9px] md:text-xs font-black text-slate-400 text-center uppercase tracking-tight truncate">${lbl}</div>`).join('')}
      </div>
    </div>
  `;

  const rowsHtml = datesList.map(dateStr => {
    const formattedDate = new Date(dateStr).toLocaleDateString(undefined, { month: 'short', day: 'numeric', weekday: 'short' });

    const cellsHtml = heatmapData[dateStr].map((block, idx) => {
      ...
      const cellTooltip = `
        <div class="flex flex-col gap-1 text-left min-w-[150px]">
          <span class="text-[9px] uppercase font-black text-slate-400 border-b border-slate-700 pb-1 mb-1">${blockLabels[idx]}</span>
          <span class="text-[10px] text-slate-300 font-bold block">${formattedDate}</span>
      ...
      return `
        <div class="flex-1 aspect-square rounded-md lg:rounded-lg ${bgClass} border transition-all duration-300 flex items-center justify-center relative group/cell cursor-pointer hover:ring-2 hover:ring-offset-2 hover:ring-emerald-500 hover:scale-105 z-10 hover:z-[9999]">
          <!-- Tooltip Popup -->
    ...
    return `
      <div class="flex items-center gap-2 md:gap-4">
        <div class="w-20 md:w-28 text-xs md:text-sm font-black text-slate-500 dark:text-slate-400 text-right shrink-0">
          ${formattedDate}
        </div>
        <div class="flex-1 flex items-center gap-1.5 md:gap-3">
          ${cellsHtml}
...
function renderSummaryCard(title, val, svgIcon, trendLabel, colorScheme, desc) {
  let bgGlow = "";
  let iconBg = "";
  let iconText = "";
  let borderHover = "";
  ...
  return `
    <div class="group relative bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800/80 rounded-3xl p-6 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 ease-out overflow-hidden flex flex-col justify-between min-h-[170px] ${borderHover} select-none">
      ...
      <div class="mt-6 z-10 flex flex-col">
        <span class="text-xs font-black uppercase tracking-wider text-slate-400 dark:text-slate-500 block mb-1">${title}</span>
