...
router.post('/', requireAuth, (req, res) => {
    ...
    ).run(sku, name, category, description || null, brand_id, req.session.user.id, buying_price || 0, stock || 0, min_stock_level || 0);
...
router.put('/:id', requireAuth, (req, res) => {
    ...
    const product = db.prepare('SELECT * FROM products WHERE id = ? AND user_id = ?').get(productId, req.session.user.id);
    if (!product) return res.status(404).json({ error: 'Product not found' });

    db.prepare(
        'UPDATE products SET sku=?, name=?, category=?, description=?, brand_id=?, buying_price=?, stock=?, min_stock_level=? WHERE id=?'
    ).run(sku, name, category, description || null, brand_id, buying_price || 0, stock ?? product.stock, min_stock_level || 0, productId);
