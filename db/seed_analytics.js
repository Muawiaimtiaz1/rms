...
function seedAnalytics() {
    ...
    db.transaction(() => {
        ...
        const shopId = db.prepare('INSERT INTO shops (name, allowed_panels) VALUES (?, ?)').run('Analytics Test Shop', allPanels).lastInsertRowid;
        ...
        // 4. Create Brands
        const brand1Id = db.prepare('INSERT INTO brands (name, user_id, shop_id) VALUES (?, ?, ?)').run('Nike', adminId, shopId).lastInsertRowid;
        const brand2Id = db.prepare('INSERT INTO brands (name, user_id, shop_id) VALUES (?, ?, ?)').run('Adidas', adminId, shopId).lastInsertRowid;

        // 5. Create Products
        const p1 = db.prepare(
            'INSERT INTO products (sku, name, category, brand_id, user_id, shop_id, buying_price, stock, min_stock_level) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)'
        ).run('NK-01', 'Nike Air', 'Shoes', brand1Id, adminId, shopId, 3000, 100, 10).lastInsertRowid;

        const p2 = db.prepare(
            'INSERT INTO products (sku, name, category, brand_id, user_id, shop_id, buying_price, stock, min_stock_level) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)'
        ).run('AD-01', 'Adidas Ultra', 'Shoes', brand2Id, adminId, shopId, 4000, 100, 10).lastInsertRowid;
        ...
        for (let i = 0; i < 24; i++) {
            ...
            console.log(`Generating sales for ${date.toLocaleString('default', { month: 'long', year: 'numeric' })}...`);
            ...
            products.forEach(prod => {
                ...
                for (let s = 0; s < salesInMonth; s++) {
                    ...
                    const saleDate = new Date(date.getFullYear(), date.getMonth(), day, hour, min, sec);
                    const saleDateStr = saleDate.toISOString().replace('T', ' ').split('.')[0];
                    ...
                    const saleId = db.prepare(`
                        ...
                        VALUES (?, ?, ?, ?, ?, ?)
                    `).run(shopId, adminId, total, discount, total, saleDateStr).lastInsertRowid;
