...
async function initPostgres() {
  ...
  try {
    ...
    if (!notificationsTableCheck.rows[0].exists) {
      console.log("🔧 Migrating database: Adding notifications tables...");
      await query(`
      ...
      `);
      console.log("✅ notifications tables added.");
    }
    await query(`
      CREATE TABLE IF NOT EXISTS notification_reads (
        notification_id INTEGER NOT NULL REFERENCES notifications(id) ON DELETE CASCADE,
    ...
    const appendRequiredPanels = async (tableName, requiredPanels, userRole = null) => {
      if (!['shops', 'users'].includes(tableName)) return 0;
      const whereSql = tableName === 'users' && userRole ? ' WHERE role = $1' : '';
      const params = tableName === 'users' && userRole ? [userRole] : [];
      const rows = await query(`SELECT id, allowed_panels FROM ${tableName}${whereSql}`, params);
      let updated = 0;
    ...
    const shopNotificationPanelUpdates = await appendRequiredPanels('shops', ['notifications']);
    const adminNotificationPanelUpdates = await appendRequiredPanels('users', ['notifications'], 'admin');
