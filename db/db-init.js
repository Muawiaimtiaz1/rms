...
async function initPostgres() {
  ...
  try {
    ...
    const financialLogColumns = [
      ["subscription_id", "INTEGER REFERENCES subscriptions(id) ON DELETE SET NULL"],
      ["updated_at", "TIMESTAMPTZ DEFAULT NOW()"],
    ];
    for (const [columnName, columnType] of financialLogColumns) {
      const columnCheck = await query(`
        SELECT column_name FROM information_schema.columns
        WHERE table_name = 'saas_financial_logs' AND column_name = $1
      `, [columnName]);
      if (columnCheck.rows.length === 0) {
        console.log(`🔧 Migrating saas_financial_logs table: Adding ${columnName} column...`);
        await query(`ALTER TABLE saas_financial_logs ADD COLUMN ${columnName} ${columnType}`);
      }
    }

    await query(`
      INSERT INTO saas_financial_logs (shop_id, subscription_id, amount, category, description, payment_method, created_at, updated_at)
      SELECT s.shop_id, s.id, s.amount, 'subscription', CONCAT('Subscription payment: ', s.type), 'Cash', s.paid_at, s.paid_at
    ...
    const brandOwnershipCheck = await query(`
      SELECT column_name FROM information_schema.columns
      WHERE table_name = 'brands' AND column_name = 'ownership_percent'
    `);
    if (brandOwnershipCheck.rows.length === 0) {
      console.log("🔧 Migrating brands table: Adding ownership_percent column...");
      await query("ALTER TABLE brands ADD COLUMN ownership_percent DOUBLE PRECISION NOT NULL DEFAULT 0");
      await query(`
        UPDATE brands b
        SET ownership_percent = 100.0 / NULLIF(counts.brand_count, 0)
        FROM (
          SELECT shop_id, COUNT(*) as brand_count
          FROM brands
          GROUP BY shop_id
        ) counts
        WHERE counts.shop_id = b.shop_id
          AND COALESCE(b.ownership_percent, 0) = 0
      `);
      console.log("✅ brands.ownership_percent added.");
    }

    const brandTypeCheck = await query(`
      SELECT column_name FROM information_schema.columns
      WHERE table_name = 'brands' AND column_name = 'partner_type'
    `);
    if (brandTypeCheck.rows.length === 0) {
      console.log("🔧 Migrating brands table: Adding partner_type column...");
      await query("ALTER TABLE brands ADD COLUMN partner_type TEXT NOT NULL DEFAULT 'share_based'");
      await query(`
        UPDATE brands
        ...
        END
      `);
      await query("UPDATE brands SET ownership_percent = 0 WHERE partner_type = 'product_based'");
      await query(`
        ALTER TABLE brands
        ADD CONSTRAINT brands_partner_type_check
        CHECK (partner_type IN ('share_based', 'product_based'))
      `);
      console.log("✅ brands.partner_type added.");
    } else {
      await query(`
        UPDATE brands
      ...
      `);
      await query("UPDATE brands SET partner_type = 'share_based' WHERE LOWER(COALESCE(name, '')) IN ('owner', 'admin')");
      await query("UPDATE brands SET ownership_percent = 0 WHERE partner_type = 'product_based'");
    }
    ...
    const productCommissionColumns = [
      ["is_commission_based", "INTEGER NOT NULL DEFAULT 0"],
      ["third_party_person_id", "INTEGER REFERENCES third_party_persons(id) ON DELETE SET NULL"],
      ["commission_percentage", "DOUBLE PRECISION NOT NULL DEFAULT 0"],
    ];
    for (const [columnName, columnType] of productCommissionColumns) {
      const columnCheck = await query(`
        SELECT column_name FROM information_schema.columns
        WHERE table_name = 'products' AND column_name = $1
      `, [columnName]);
      if (columnCheck.rows.length === 0) {
        console.log(`🔧 Migrating products table: Adding ${columnName} column...`);
        await query(`ALTER TABLE products ADD COLUMN ${columnName} ${columnType}`);
        console.log(`✅ products.${columnName} added.`);
      }
    }

    const saleItemCommissionColumns = [
      ["third_party_person_id", "INTEGER REFERENCES third_party_persons(id) ON DELETE SET NULL"],
      ["commission_percentage_at_sale", "DOUBLE PRECISION NOT NULL DEFAULT 0"],
      ["commission_amount_at_sale", "DOUBLE PRECISION NOT NULL DEFAULT 0"],
    ];
    for (const [columnName, columnType] of saleItemCommissionColumns) {
      const columnCheck = await query(`
        SELECT column_name FROM information_schema.columns
        WHERE table_name = 'sale_items' AND column_name = $1
      `, [columnName]);
      if (columnCheck.rows.length === 0) {
        console.log(`🔧 Migrating sale_items table: Adding ${columnName} column...`);
        await query(`ALTER TABLE sale_items ADD COLUMN ${columnName} ${columnType}`);
        console.log(`✅ sale_items.${columnName} added.`);
      }
    }
    ...
    await query(`
      ...
      CREATE TABLE IF NOT EXISTS partner_ownership_history (
        id INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
        shop_id INTEGER NOT NULL REFERENCES shops(id) ON DELETE CASCADE,
        brand_id INTEGER NOT NULL,
        partner_name TEXT NOT NULL,
        ownership_percent DOUBLE PRECISION NOT NULL CHECK (ownership_percent >= 0 AND ownership_percent <= 100),
        effective_from TIMESTAMPTZ NOT NULL,
        effective_to TIMESTAMPTZ,
        created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
        CHECK (effective_to IS NULL OR effective_to > effective_from)
      );
      CREATE INDEX IF NOT EXISTS idx_partner_ownership_history_lookup
        ON partner_ownership_history(shop_id, effective_from, effective_to, brand_id);
    `);
    await query(`
      INSERT INTO partner_ownership_history (shop_id, brand_id, partner_name, ownership_percent, effective_from)
      SELECT b.shop_id, b.id, b.name, b.ownership_percent, TIMESTAMPTZ '2000-01-01 00:00:00+00'
      FROM brands b
      WHERE NOT EXISTS (
        SELECT 1 FROM partner_ownership_history h WHERE h.shop_id = b.shop_id
      )
    `);
    ...
    await query(`
      ...
      SET
        ...
        product_count = COALESCE((SELECT COUNT(*) FROM products p WHERE p.shop_id = shops.id), 0)
    `);
